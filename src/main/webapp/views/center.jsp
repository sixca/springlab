...
<%-- 던져진 JSON을 서버에 요청해서 해체 뿌려야지 weather2를 수행하기 위한 AJAX --%>
  let center = {
    ...
    display:function (data){
      ...
      // alert(result);  //확인용. 기상예보 멘트인 string이 result니깐 textarea에 바로 뿌림
      // $('#w2').html(result);  weather2

      //weather3
      var result = data.response.body.items.item; //weather3. item은 [] 배열임
      var txt = '';
      $(result).each(function (index, item){   //[]배열을 foreach문으로 돌리는 제이쿼리 :: item이 db단에서 애초에 배열임
        txt += '<h5>';
        txt += '일시: ' + item.tm + ' ' + '기온: ' + item.ta;
        txt += '</h5>';
      });
      $('#w2').html(txt);
      //textarea에는 html이 못들어가니깐 div를 따로 만들어서 넣어줌
